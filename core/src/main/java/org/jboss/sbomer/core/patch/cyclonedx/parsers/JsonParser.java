 ...
 */
package org.jboss.sbomer.core.patch.cyclonedx.parsers;
...
public class JsonParser extends CycloneDxSchema implements Parser {
     ...
     * {@inheritDoc}
     */
    public List<ParseException> validate(final InputStream inputStream, final Version schemaVersion)
            throws IOException {
    ...
    public List<ParseException> validate(final JsonNode bomJson, final Version schemaVersion) throws IOException {
        ...
        if (!schemaVersion.getFormats().contains(Format.JSON)) {
            exceptions.add(
                    new ParseException(
                            "CycloneDX version " + schemaVersion.getVersionString()
                                    + " does not support the JSON format"));
        }

        Set<ValidationMessage> errors = getJsonSchema(schemaVersion, mapper)
                .validate(mapper.readTree(bomJson.toString()));
