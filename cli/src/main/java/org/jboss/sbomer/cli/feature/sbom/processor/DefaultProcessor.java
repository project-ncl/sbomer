...
public class DefaultProcessor implements Processor {
    ...
    private void addMissingNpmDependencies(Bom bom) {
        ...
        if (externalReferences.size() > 1) {
            log.warn("Component {} hase more then one {}/{} external reference", component.getPurl(), ExternalReference.Type.BUILD_SYSTEM, SBOM_RED_HAT_PNC_BUILD_ID);
            return;
        }
    ...
    private void addMainComponentDependencies(Bom bom, Set<String> dependencies) {
        ...
        dependencies.stream().map(SbomUtils::createDependency).forEach(mainComponentDependencies::addDependency);
    }
